...
We recall that \emph{Baxter permutations} are permutations avoiding the vincular pattern $2\underline{41}3$ and $3\underline{14}2$, where the underlining indicates that the symbols must appear contiguously in the permutation.
...
\begin{proof}
  ...
  Since the weak poset extends the order induced by $T(R)$, we have that either $\ell=i$ or $\ell\prec_s i$, hence $\pi^{-1}(\ell)\leq \pi^{-1}(i)$.
...
A symmetric analogue of Lemma~\ref{lem:weakCoord} holds for down shard polytopes, where the product contributes to $-\b{e}_i$ instead of $\b{e}_i$.
This proves Theorem~\ref{thm:weakRectangulotope}.
...
From the results of section~\ref{subsec:quotientopes}, the submodular function corresponding to our realization of $\SRP(n)$ can be obtained as before by summing the functions defining the yin and yang shard polytopes in \cref{lem:yinsp,lem:yangsp}.
