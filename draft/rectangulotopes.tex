...
\usepackage{enumerate, amsmath, amsfonts, amssymb, amsthm, mathrsfs, wasysym, graphics, graphicx, xcolor, url, hyperref, hypcap, shuffle, xargs, multicol, overpic, pdflscape, multirow, hvfloat, minibox, accents, array, multido, xifthen, a4wide, ae, aecompl, blkarray, pifont, mathtools, etoolbox, dsfont, verbatim}
...
\newcommand{\WRP}{\polytope{WR}} % weak rectangulation polytope
\newcommand{\SRP}{\polytope{SR}} % strong rectangulation polytope
...
\begin{theorem}
  Let $\polytope{P} (n)$ be the convex hull of the points
  $\sum_{i\in [n]} \pi^{-1}(i)\cdot \mathbf{e}_i$, 
  for all permutations $\pi\in\f{S}_n$.
  Then $\polytope{P} (n)$ is a realization of the $(n-1)$-dimensional permutohedron.
  \end{theorem}


\subsection{Loday's associahedra}

In 2004, Loday published the following elegant realization of the ubiquitous associahedron~\cite{}.
...
\begin{itemize}
\item Terminology: rectangles, vertices, segments, generic rectangulation
\item Wall slides
...
\begin{theorem}
  ...
  \yang{w}_{i,j} := 
  \begin{cases}
    v(S(R), j) \cdot ch (T(R), S(R), i, j)\cdot v(T(R), i) & \text{ if neither } j|i \text{ nor } \frac{\ i\ }j, \\
    0 & \text{ otherwise.}
  \end{cases}
  \]
  ...
  \end{itemize}
Then $\SRP (n)$ is a realization of the $(n-1)$-dimensional strong rectangulation polytope.  
\end{theorem}
...
We define the \defn{weak poset} $P_w(R)=([n],\prec_w)$ of a rectangulation $R$ as the transitive closure of the union of the two partial orders defined by the intertwining binary trees $T(R)$ and $S(R)$.
...
The weak rectangulation polytope $\WRP(n)$ is the quotientope of the Baxter congruence of the weak Bruhat order on $\f{S}_n$.

\subsection{Loday and anti-Loday arcs}

The Baxter congruence is defined by forbidding arcs that cross the horizontal line.
Therefore, the quotientope is defined by a Minkowski sum of the shard polytopes defined by two families of arcs: the \defn{Loday arcs} and the \defn{anti-Loday arcs}, that go respectively above and below all elements of an interval $I$ of $[n]$.

\begin{lemma}
  Given a Loday arc $A$ defined by an interval $I$, the Loday shard polytope $\SP(A)$ is defined as the standard simplex $\conv \{ \mathbf{e}_i : i\in I \}$.
\end{lemma}

\begin{lemma}
  Given an anti-Loday arc $A$ defined by an interval $I$, the anti-Loday shard polytope $\SP(A)$ is defined as the negative standard simplex $\conv \{ - \mathbf{e}_i : i\in I \}$.
\end{lemma}
...
\begin{theorem}
  ...
  are equivalence classes of a congruence relation on the weak Bruhat order on $\f{S}_n$.
  The minimal elements of each equivalence classes are 2-clumped permutations, and the maximal
  elements are co-2-clumped permutations.
\end{theorem}

The strong rectangulation polytope $\SRP(n)$ is the quotientope of the congruence of the weak Bruhat order on $\f{S}_n$ that is defined by the 2-clumped permutations.

\jean{Does the congruence have a name?}

\subsection{Yin and yang arcs}

The strong rectangulation congruence is defined by forbidding the arcs that cross the horizontal line more than once. We classify the remaining arcs into two families, depending on whether the arcs cross from above or below.

A \defn{yin arc} is defined by two consecutive nonempty intervals $I$ and $J$ of $[n]$ such that the arc goes above all elements of $I$ and
below all elements of $J$. Note that the Loday arc are obtained when $|J|=1$ and the anti-Loday arc are obtained with $|I|=1$.
A \defn{yang arc} is defined similarly, except that the arc goes below the elements of $I$ and above the elements of $J$. Note that Loday and anti-Loday arcs are also yang arcs.

\begin{lemma}
  Given a yin arc $A=(I,J)$, the shard polytope $\SP(A)$ is defined as the convex hull of the points
  $\{\mathbf{0} \}\cup \{ \mathbf{e}_i - \mathbf{e}_j : (i,j)\in I\times J\}$.
  %  \SP(A) = \conv (\{\mathbf{0} \}\cup \{ \mathbf{e}_i - \mathbf{e}_j : (i,j)\in I\times J\})
\end{lemma}

\begin{lemma}
  Given a yang arc $A=(I,J)$, the shard polytope $\SP(A)$ is defined as the convex hull of the points
  $\{\mathbf{0} \}\cup \{ \mathbf{e}_j - \mathbf{e}_i : (i,j)\in I\times J\}$.
  %  \SP(A) = \conv (\{\mathbf{0} \}\cup \{ \mathbf{e}_j - \mathbf{e}_i : (i,j)\in I\times J\})
\end{lemma}
...
Note that since Loday and anti-Loday arcs are both yin and yang, each Loday and anti-Loday shard polytope is summed twice in the proposed realization. This does not change the combinatorial type of the polytope.

\subsection{Loday-type coordinates for strong rectangulation polytopes}

For every rectangulation $R$, we wish to give the coordinates of a point $p(R)\in\R^n$ such that
$\SRP(n)$ is the convex hull of the points $p(R)$ for all rectangulations $R$.
