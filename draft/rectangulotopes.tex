...
\weakeq}{\asymp}
%\makeatletter
%\DeclareRobustCommand{\strongeq}{%
%  \mathbin{\mathpalette\scaletoplus@{\mkern1mu \smash{\begin{smallmatrix} \backsim \\[-.3cm] \sim \end{smallmatrix}} \mkern1mu}}%
%}
...
The goal of this paper is to describe the quotientopes of congruences of the weak Bruhat order whose classes are in bijection with equivalence classes of rectangulations, defined as decompositions of a square into rectangles.

\subsection{Weak and strong rectangulations}

The combinatorics of rectangulations has been studied for decades~\cite{MR2233287,MR2871762,MR2864445,MR2763051,MR3084577,MR3878132,MR4598046}.
Interestingly, many combinatorial results were initially motivated by applications to the design of \defn{floorplans} for very large scale integrated circuits, and published in electrical engineering journals~\cite{}.
The recent order-theoretic take on these structures has proved fruitful.
...
First, we say that a rectangle $r$ is \defn{above} another rectangle $s$ (and $s$ is \defn{below} $r$) if either the bottom edge of $r$ lies in the same segment as the top edge of $s$, or if $r$ is above another rectangle $t$ that is above $s$.
...
\begin{figure}
  \caption{\label{fig:rectequiv}Weak and strong equivalence between rectangulations.}
\end{figure}
...
The \defn{weak rectangulation congruence} (or \defn{Baxter congruence}) was first explicitly studied by S.~Law and N.~Reading~\cite{MR2871762}.
Its classes are one-to-one with weak rectangulations, but also with \defn{twin binary trees}~\cite{Giraudo}, and with \defn{Baxter permutations}~\cite{}. % more baxter refs
The corresponding quotientopes will be referred to as the \defn{weak rectangulotopes} and denoted by $\WRP(n)$.
On the other hand, the \defn{strong rectangulation congruence}, whose classes are in bijection with strong rectangulations~\cite{MR2864445}.
The latter is mentioned by N.~Reading~\cite{MR3335492} (see for instance in Theorem 1.2, and Example 4.11), and studied more recently by A.~Asinowski, J.~Cardinal, S.~Felsner, and É.~Fusy~\cite{ACFF24}.
...
We refer to S.~Law and N.~Reading~\cite{MR2871762}, J.~Cardinal, V.~Sacrist\'an, and R.~Silveira~\cite{}, E.~Meehan~\cite{MR3697823}, and A.~Asinowski, J.~Cardinal, S.~Felsner, and \'E.~Fusy~\cite{ACFF24} for detailed descriptions of the flip graphs on both weak and strong rectangulations.
...
\begin{lemma}
  The skeleton of $\WRP(n)$ is isomorphic to the flip graph on weak rectangulations of size $n$.
  Similarly, the skeleton of $\SRP(n)$ is isomorphic to the flip graph on strong rectangulations of size $n$.
\end{lemma}
...
\begin{theorem}
  ...
  $\sum_{i\in [n]} (\loday{w}^R_i - \antiloday{w}^R_i)\cdot \b{e}_i$ for all weak rectangulations $R$ on $n$ rectangles,
  with
  \[
%  \begin{split}
...
Therefore, the quotientope is realized by a Minkowski sum of the shard polytopes defined by two families of arcs: the \defn{up arcs} and the \defn{down arcs}, that go respectively above and below all elements of an interval $I$ of $[n]$.

\begin{lemma}
  \label{lem:lodaysp}
  Given a up arc $A$ defined by an interval $I$, the up shard polytope $\SP(A)$ is a translate of the standard simplex $\conv \set{ \b{e}_i }{ i\in I }$.
\end{lemma}

\begin{lemma}
  \label{lem:antilodaysp}
  Given a down arc $A$ defined by an interval $I$, the down shard polytope $\SP(A)$ is a translate of the negative standard simplex $\conv \set{ - \b{e}_i }{ i\in I }$.
\end{lemma}
...
\vincent{Say that this was already known by Law and Reading.}
...
\begin{lemma}
  ...
  \[
  f(S) \eqdef \#\bigset{ I\text{ interval of } [n] }{ I\cap S\not=\emptyset \text{ and } I\cap ([n]\ssm S) \not= \emptyset }.
  \]
  %that maps $S$ to the number of intervals $I$ of $[n]$ such that $I\cap S\not=\emptyset$ and $I\not\subseteq S$. 
\end{lemma}

\begin{proof}
  From \cref{lem:weakMinkowski,lem:submodsum}, the submodular function defining the realization of $\WRP(n)$ is the sum of the submodular functions defining the up and down shard polytope in \cref{lem:lodaysp,lem:antilodaysp}. For a up arc $A$ defined by an interval $I$ of $[n]$, we have that its shard polytope $\SP(A)$ is realized by the submodular function
  \[
  \loday{f}_I(S) \eqdef \llbracket S\cap I\not=\emptyset \rrbracket .
  \]
  (Recall that $\llbracket \varphi\rrbracket$ is equal to 1 if $\varphi$ holds, and 0 otherwise.)
  Similarly, for a down arc $A$ defined by an interval $I$, the shard polytope $\SP(A)$ is defined by the following submodular function:
  \[
  \antiloday{f}_I(S) \eqdef - \llbracket I\subseteq S \rrbracket .
  \]
  Their sum
  \[
  f(S) \eqdef \sum_{I\text{ interval of }[n]} \loday{f}_I(S) + \antiloday{f}_I(S)
  \]
...
\begin{lemma}
  \label{lem:lodaymax}
  Let $\pi\in\f{S}_n$ and consider a up arc $A$ defined by an interval $I$.
  ...
  Similarly, if $A$ is a down arc defined by $I$, the unique vertex of $\SP(A)$ that is extreme with respect to the direction $\pi^{-1}$
...
\begin{lemma}
  ...
  Given $i\in [n]$, the number of up arcs $A$ such that $\b{e}_i$ is the extremal vertex of $\SP(A)$ contributing to $p(R)$ is
  \[
...
Also note that since up and down shard polytopes are, up to a translation, both yin and yang, each up and down shard polytope is summed twice in the proposed realization.
