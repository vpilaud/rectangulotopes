...
%
We denote by ${}^i \!\!\! \raisebox{.03cm}{\verticalPattern} \!\!\! {}_j$ the situation where the rectangles $r_i$ and $r_j$ of inorder labels $i$ and $j$ share a vertical segment containing the right edge of~$r_i$ and the left edge of~$r_j$, and the bottom edge of $r_i$ is below the top edge of~$r_j$.
...
We use the Iverson bracket $\llbracket \varphi\rrbracket$, which is equal to 1 if $\varphi$ holds, and 0 otherwise.
...
The maximal cone corresponding to a binary tree~$T$ is given by~$\polytope{C}^T = \set{\b{x} \in \R^n}{x_i \le x_i \text{ for each edge } i \to j \text{ in } T}$.
...
This, in passing, gives us alternative characterizations of the sylvester congruence $\frown$ and the weak rectangulation congruence $\weakeq$:
$\sigma \frown \tau$ if and only if for any interval $I$ of $[n]$,
$\arg\max_{j\in I} \sigma^{-1}_j=\arg\max_{j\in I} \tau^{-1}_j$, and
$\sigma \weakeq \tau$ if the previous property holds, and in addition
$\arg\min_{j\in I} \sigma^{-1}_j=\arg\min_{j\in I} \tau^{-1}_j$.
...
\begin{proof}
  ...
  Since the weak poset extends the order induced by $T$, we have that either $\ell=i$ or $\ell\prec_s^R i$, hence $\sigma^{-1}_\ell \leq \sigma^{-1}_i$.
...
\begin{lemma}
  \label{lem:yinminmax}
  Let $\sigma\in\f{S}_n$, let~$I$ and~$J$ be two consecutive intervals of~$[n]$, and let $i\eqdef \arg\max_{k\in I} \sigma^{-1}_k$ and $j \eqdef \arg\min_{k\in J} \sigma^{-1}_k$ (respectively, for yang arcs, $i\eqdef \arg\min_{k\in I} \sigma^{-1}_k$ and $j \eqdef \arg\max_{k\in J} \sigma^{-1}_k$).
...
$\arg\max_{k\in I} \sigma^{-1}_k = \arg\max_{k\in I} \tau^{-1}_k$, 
$\arg\min_{k\in J} \sigma^{-1}_k = \arg\min_{k\in J} \tau^{-1}_k$,
...
A similar statement holds for the yang congruence $\backsim$, by exchanging $\min$ and $\max$, and flipping the sign of the inequality.
