...
The latter is mentioned by N.~Reading~\cite{Reading-arcDiagrams} (see for instance in Theorem 1.2, and Example 4.11), and studied more recently by A.~Asinowski, J.~Cardinal, S.~Felsner, and É.~Fusy~\cite{ACFF24}.
...
\begin{figure}
	...
	\caption{The weak Bruhat order on~$\f{S}_4$ (left), the sylvester congruence~$\equiv_\textrm{sylv}$~(middle), and the Tamari lattice (right). \cite[Fig.~1 \& 2]{PilaudSantos-quotientopes}}
...
For instance, the \defn{sylvester congruence} is ... \vincent{todo}
...
\begin{figure}
	...
	\caption{The braid fan (left) and the sylvester fan (right). \cite[Fig.~1]{PilaudSantos-quotientopes}}
	\label{fig:fans}
\end{figure}
...
\begin{figure}
	...
	\caption{The permutahedron~$\Perm(4)$ (left) and the associahedron~$\Asso(4)$. \cite[Fig.~1]{PilaudSantos-quotientopes}}
...
\begin{lemma}
  Let $R$ be a rectangulation on~$n$ rectangles, with source and target trees~$S$ and~$T$, and let $\b{p}(R)$ the vertex of $\WRP(n)$ associated with $R$.
...
\begin{lemma}
  Let $R$ be a strong rectangulation on $n$ rectangles, $S$ and $T$ be its source and target trees, $([n],\prec_s)$ be its strong order, and $\b{p}(R)$ be the vertex of $\SRP(n)$ corresponding to $R$.
...
\begin{lemma}
  Let $R$ be a rectangulation on $n$ rectangles, $S$ and $T$ be its source and target trees, and $([n],\prec_s)$ be the associated strong order.
