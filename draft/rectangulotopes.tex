...
The weak rectangulotope $\WRP (n)$ is realized by: \vincent{We loose the information that the two descriptions define the same polytope...}
  \begin{enumerate}[(i)]
  \item the convex hull of the points
    \[
...
\end{example}
\end{comment}
\vincent{not sure if we want to reintroduce this example.}
...
We denote by $i\verticalPattern j$ the situation where 
...
\vincent{Add picture weak and strong orders?}
\begin{enumerate}
...
\begin{lemma}
  \label{lem:yincount}
  Let $R$ be a strong rectangulation of size~$n$, let~$S$ and $T$ be its source and target trees, let~$([n],\prec_s)$ be its strong poset, and let~$\b{p}(R)$ be the vertex of $\SRP(n)$ corresponding to $R$.
...
Recall that $i\verticalPattern j$ denote the situation where the rectangles $r_i$ and $r_j$ of inorder labels $i$ and $j$ are such that $r_i$ and $r_j$ share a vertical segment, $r_i$ is on the left of $r_j$, and the bottom edge of $r_i$ is lower than the top edge of $r_j$.
...
For that purpose, we prove that for every pair $i<j\in [n]$ such that $i\prec_s j$, hence which do not have any associated yin arc, we have either $cv^{T,S}_{i,j} = 0$ (hence the product in \cref{lem:yincount} is 0 as well and adding it does not harm the result), or $i\verticalPattern j$. 
...
\begin{proof}
  ...
  We now claim that there can only be one segment on which leaves common to the vertical subtree of $i$ in $T$ and the vertical subtree of $j$ in $S$ may lie, and this segment is the vertical segment supporting the right edge of $r_i$ and the left edge of $r_j$.
  \Jean{This requires a case analysis. Any shortcut?}
  
  Let us assume that this segment is shared.
  ...
  Hence we must have that the bottom edge of $r_i$ is lower than the top edge of $r_j$, hence that $i\verticalPattern j$, as claimed.
\end{proof}

We omit the proofs of the analogues of \cref{lem:yincount} and \cref{lem:whentosum} for the case of the yang arcs: The arguments are completely symmetric.
