...
\begin{figure}
  \includegraphics[width=.9\textwidth]{rectangulations.pdf}
	...
	\label{fig:rectequiv}
\end{figure}
\vincent{Add one which is not weakly equivalent?}
...
We recall that \emph{Baxter permutations} are permutations avoiding the vincular patterns $2\underline{41}3$ and $3\underline{14}2$, where the underlining indicates that the symbols must appear contiguously in the permutation.
...
This proves \cref{thm:weakRectangulotope}.
...
\item The \defn{special relation} $([n],\btri)$ of $R$, in which $a\btri b$ if the two rectangles with inorder labels $a$ and $b$ are such that 
	\begin{enumerate}[(i)]
	\item either the right side of $b$ and the left side of $a$ lie on the same vertical segment, and the bottom-right corner of $b$ is above the top-left corner of $a$, 
	\item or the top side of $b$ and the bottom side of rectangle $a$ lie on the same horizontal segment, and the top-left corner of $b$ lies on the right of the bottom-right corner~of~$a$.
	\end{enumerate}
\end{enumerate}
...
%The following lemmas are direct applications of the results from~\cite{MR4584712} given in \cref{subsec:quotientopes}.
%See \cref{fig:shardPolytopes}.
