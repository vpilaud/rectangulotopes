...
$\sigma \weakeq \tau$ if and only if for any interval $I$ of $[n]$,
...
\begin{lemma}
  \label{lem:yinminmax}
  Let $\sigma\in\f{S}_n$, let~$I$ and~$J$ be two consecutive intervals of~$[n]$, and let $i \eqdef \arg\max_{k\in I} \sigma^{-1}_k$ and $j \eqdef \arg\min_{k\in J} \sigma^{-1}_k$.
  The unique vertex of the yin shard polytope~$\SP(\yinArc{I}{J})$ that is extreme with respect to the direction $\sigma^{-1}$ is $\b{e}_i-\b{e}_j$ if $\sigma^{-1}_i > \sigma^{-1}_j$, and $\b{0}$ otherwise.
\end{lemma}
