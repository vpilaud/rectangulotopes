...
\begin{abstract}
  ...
  We give simple realizations of $(n-1)$-dimensional polytopes associated with rectangulations composed of $n$ rectangles. They correspond to quotientopes of natural congruence relations on the weak Bruhat order on permutations in $\f{S}_n$, and their skeleta are flip graphs on rectangulations. We give  elementary formulas for computing the coordinates of each vertex of the polytope, in the spirit of Loday's realization of the associahedra.
\end{abstract}
...
\begin{itemize}
\item Terminology: Rectangles, vertices, segments
\item Wall slides
\item Two equivalence classes: weak equivalence preserving rectangle-segment adjacencies, and strong equivalence preserving rectangle-rectangle adjacencies.
\item Weak equivalence classes have a \emph{diagonal} representative.
\end{itemize}
...
\begin{theorem}
  ...
  $\sum_{i\in [n]} w_i\cdot \mathbf{e}_i$,
  where
  \[
  \begin{split}
    w_i := \quad & h(T(R), i)\cdot v(T(R),i) \\
           \quad & - h(S(R), i)\cdot v(S(R),i),
  \end{split}
  \]
...
Our second main result on the realizations of the strong rectangulation polytopes does not require any additional definition. It is, however, slightly more involved.

\begin{theorem}
  Let $\SRP (n)$ be the convex hull of the points
  \[
  \sum_{i,j\in [n]} w_{i,j}\cdot (\mathbf{e}_i - \mathbf{e}_j),
  \]
  with
  \[
  \begin{split}
  w_{i,j} := \quad & h(T(R), i) \cdot cv (T(R), S(R), i, j)\cdot h(S(R), j) \\
            \quad & - v(S(R), j) \cdot ch (T(R), S(R), i, j)\cdot v(T(R), i)
  \end{split}
  \]
  \jean{Write the correct formula here, and sum only on the suitable pairs $i,j$.}
  ...
  \item $ch (T,S,i,j)$ and $cv (T,S,i,j)$ denote respectively the number of \emph{common leaves} to the horizontal (vertical) subtree of $i$ in $T$ and the horizontal (vertical) subtree of $j$ in $S$
  \end{itemize}
...
\jean{State the main results that we will use. I just give sloppy statements for now.}
...
Define the weak poset $P_w(R)$ of a rectangulation $R$ as the intersection of two partial orders defined by the twin binary trees

\begin{theorem}
  The following subsets of $\f{S}_n$ 
  \[
  \{\mathcal{L}(P_w(R)) : R \text{ is a weak rectangulation} \}
  \]
...
We call this congruence the \emph{Baxter congruence}.
The weak rectangulation polytope $\WRP(n)$ is the quotientope of the Baxter congruence of the weak Bruhat order on permutations.

\subsection{Loday and anti-Loday arcs}

The Baxter congruence is defined by forbidding arcs that cross the horizontal line.
Therefore, the quotientope is defined by a Minkowski sum of the shard polytopes defined by two families of arcs: the Loday and the anti-Loday arcs.
...
Define the strong poset $P_s(R)$ of a rectangulation $R$ using the intertwined binary trees.

\begin{theorem}
  The following subsets of $\f{S}_n$ 
  \[
  \{\mathcal{L}(P_s(R)) : R \text{ is a strong rectangulation} \}
  \]
  are equivalence classes of a congruence relation on the weak Bruhat order on $\f{S}_n$.
...
The strong rectangulation polytope $\SRP(n)$ is the quotientope of the congruence of the weak Bruhat order on permutations that is defined by the 2-clumped permutations.

\jean{Does the congruence have a little name?}
...
\begin{itemize}
\item Identify what are the vertices of each yin summands that contribute to a given rectangulation,
\item group the contributions for each pair of rectangles $i,j$,
