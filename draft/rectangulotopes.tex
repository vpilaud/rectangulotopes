...
\Vincent{I would change this theorem...}
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
  \item the convex hull of the points~$\sum_{i\in [n]} \ell^T_i\cdot r^T_i \cdot \mathbf{e}_i$ for all binary trees~$T$ with $n$ internal nodes, where $\ell^T_i$ and $r^T_i$ denote the number of leaves in the left and right subtrees of $i$ in $T$,
  \item the intersection of the hyperplane defined by~$\sum_{i \in [n]} x_i = \binom{n+1}{2}$ with the halfspaces defined by~$\sum_{i \in S} x_i \le \#\set{I \text{ interval of } [n]}{S \cap I \ne \varnothing}$.
  \end{enumerate}
\end{theorem}
...
The description of the weak rectangulotope $\WRP(n)$ as a Minkowski sum in Lemma~\ref{lem:weakMinkowski} allows us to easily compute the submodular function defining it.
