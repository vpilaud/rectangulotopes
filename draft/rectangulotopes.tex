...
In order to focus on the combinatorial structure of a rectangulation, we recall two equivalence relations between rectangulations~\cite{??}.
...
This is illustrated in \cref{fig:rectequiv}.
A \defn{diagonal rectangulation} is a rectangulation in which every rectangle intersects the top-left to bottom-right diagonal of the square, see \cref{fig:weakRectangulation}.
...
\item \defn{Pivoting operations} transform a pair of adjacent rectangles whose union is not a rectangle, changing a left-right pair into an above-below pair, and vice-versa,
...
%  \end{split}
  ...
  \item the intersection of the hyperplane defined by~$\sum_{i \in [n]} x_i = 0$ with the halfspaces defined by \linebreak $\sum_{i \in S} x_i \le \#\set{I \text{ interval of } [n]}{I \not\subseteq S \text{ and } I \not\subseteq [n] \ssm S}$ for all subsets~$\varnothing \ne S \subsetneq [n]$.
  \end{enumerate}
\end{theorem}

\begin{example}
The weak rectangulotope~$\WRP(4)$ is illustrated in \cref{fig:weakRectangulotope}.
The coordinates of the vertex corresponding to each weak rectangulation are gathered in \cref{tab:verticesWeakRectangulotope}.
...
The coordinates of the vertex corresponding to each strong rectangulation are gathered in \cref{tab:verticesStrongRectangulotope}.
...
The \defn{weak Bruhat order}\footnote{The weak Bruhat order is usually just referred to as the weak order~\cite{Humphrey, BjornerBrenti}. In this paper, we prefer to use weak Bruhat order to avoid any confusion with the weak poset defined on weak rectangulations.} is the lattice on the permutations of~$\f{S}_n$ defined by the inclusion of their inversion sets.
...
The strong poset is a two-dimensional lattice, whose minimum is the root of~$S(R)$ and whose maximum is the root of~$T(R)$.
...
\begin{proof}
  ...
  For this, we consider the diagonal representative $D$ of the weak equivalence class of $R$, and recall that the weak poset of $R$ is the adjacency poset of $D$.
  ...
  Note that each of these we can only add new adjacencies, hence the adjacency poset of $R$ is always an extension of that of $D$. 
\end{proof}
...
\end{lemma}
\begin{proof}
  Let $\pi$ be any permutation corresponding to $R$, hence an element of $\mathcal{L}(P_s(R))$.
  ...
  number $h^T_i$ of leaves of the horizontal subtree of $T(R)$ rooted at $i$, the number $cv^{T,S}_{i,j}$ of common leaves to the vertical subtrees of $T(R)$ and $S(R)$ rooted
  at $i$ and $j$, respectively, and the number $h^S_j$ of leaves of the horizontal subtree of $S(R)$ rooted at $j$.

  We first show that the number of choices for the left endpoint of $I$ is equal to the number $h^T_i$ of leaves in the horizontal subtree of $i$ in $T(R)$.
  ...
  Since from Lemma~\ref{lem:sextw} the strong poset extends the order induced by $T(R)$, we have that either $\ell=i$ or $\ell\prec_s i$, hence $\pi^{-1}(\ell)\leq \pi^{-1}(i)$.
  Therefore, each such choice of left endpoint for $I$ preserves the fact that $i$ is the index of the maximum of $\pi^{-1}$ in $I$.
  Now consider the leaf of $T(R)$ that is immediately left of the leftmost leaf of the horizontal subtree of $i$.
  This leaf is associated with the node $\ell$ that comes next in inorder traversal, which must be an ancestor of $i$ in $T(R)$.
  ...
  First, the right endpoint of $I$ can only be chosen among the leaves of the vertical subtree of $T(R)$ rooted at $i$.
  Then the same reasoning holds for the two endpoints of $J$, replacing $i$ by $j$ and $T(R)$ by $S(R)$.
